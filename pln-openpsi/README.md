...
The implementation focuses on two fundamental PLN inference rules: **Deduction** and **Conditional Conjunction Introduction**.
...
1.  **Deduction (`deduction.metta`)**
    ...
    - **Logic**: The `pln-deduction` function iterates through pairs of rules, and if a match is found, it uses the `deduction-formula` (from `utils/deduction-utils.metta`) to calculate the STV of the newly inferred rule.
...
The reasoning process is orchestrated by `pln.metta`:

1.  A set of initial rules is loaded into a MeTTa space (`&ruleSpace`).
2.  The main `pln-reasoning` function is called.
...
-   **`pln.metta`**: The main entry point that orchestrates the overall PLN reasoning process by combining deduction and conjunction.
...
-   **`reasoned-output.metta`**: Contains examples of new rules generated by the reasoning engine.

### Utilities (`utils/`)

This directory contains essential helper functions and definitions:

-   **`deduction-engine.metta`**: Defines the core data structures (`STV`, `TruthValue`), constants, and mathematical formulas for PLN, including the `simple-deduction-strength-formula`. This is the mathematical foundation of the reasoning system.
-   **`deduction-utils.metta`**: Provides helper functions for the deduction process, such as finding matching rules to form a deductive chain.
...
-   **`rule-ops.metta`**: A crucial API for interacting with rules. It provides a suite of `extractRule` functions to reliably access specific parts of a rule (e.g., `Context`, `Goal`, `STV`).
-   **`util.metta`**: General-purpose utility functions, such as list comparison (`areSimilar`) and cleanup (`clearSpace`).
...
You can also run the inference rules individually by importing `deduction.metta` or `cond-conj-intro.metta` and calling `!(pln-deduction &ruleSpace)` or `!(pln-conjunction &ruleSpace)`.
